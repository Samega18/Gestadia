...
import com.milos.gestadia.config.KafkaSseConsumer;
import com.milos.gestadia.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/gestadia-kafka")
public class GestadiaKafkaResource {

    private final Logger log = LoggerFactory.getLogger(GestadiaKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public GestadiaKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
